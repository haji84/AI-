...
```json
{
  ...
  "evidence_refs": [],
  "status": "PARTIAL",
  "blocker": "ZBook型式・battery/UPS継続・BIOS AC Restoreを未観測。制約をPLATFORM_LIMITEDで隠さない。",
  "platform_limit": null,
  "fallback": null,
  "next_action": "P2: 短時間停電はZBook battery/UPSでの継続を考慮。 について実装の不足を埋め、required_evidenceを取得する。",
  "last_verified_commit": null
}
```
...
```json
{
  ...
  "platform_limit": null,
  "fallback": null,
  "next_action": "P2: AC断→復電→BIOS/UEFI AC Restore→Windows Boot→Tailscale unattended→JARVIS automatic startup→Broker/Gateway→Android reconnect→Remote access recovery。 について実装の不足を埋め、required_evidenceを取得する。",
  "last_verified_commit": null
}
```
...
```json
{
  ...
  "platform_limit": null,
  "fallback": null,
  "next_action": "P2: BIOS設定が必要な場合はコードで可能と偽らず一度だけ必要なHuman Gateを明示。 について実装の不足を埋め、required_evidenceを取得する。",
  "last_verified_commit": null
}
```
