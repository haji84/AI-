...
```json
{
  ...
  "platform_limit": null,
  "fallback": null,
  "next_action": "Read-only candidate generation is wired to the authenticated teaching library. Add explicit correction observations, governed skill write-back and full device acceptance; candidate validation grants no execution authority.",
  "last_verified_commit": null
}
```
