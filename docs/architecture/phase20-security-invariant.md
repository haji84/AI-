...
The product boundary is modeled against concrete abuse paths, not only component failure. The table below defines the minimum P8 threat coverage for private ingress, fleet execution, Remote Assist, recovery, privacy, protected actions, secrets, and provider routing.

| Surface | Abuse case | Required mitigation | Residual/evidence limit |
| --- | --- | --- | --- |
| Private ingress | An attacker or misconfiguration exposes Broker, enrollment, Dashboard, or Remote Gateway publicly, enables Funnel, or treats loss of private connectivity as permission to open a fallback path. | Control surfaces remain loopback/private-overlay only, public Funnel is prohibited, owner authentication stays mandatory, and loss of private connectivity fails closed rather than widening ingress. | Repository tests can prove configuration and fail-closed logic, but cellular reachability and actual network exposure still require deployment/physical evidence. |
| Fleet identity and capability | A spoofed, revoked, replaced, or mislabeled worker impersonates an enrolled device, reuses another device's capability set, or self-asserts a stronger capability. | Signed identity is bound to the enrolled public key, server-side allowlists and capability policy remain authoritative, revoked/mismatched identities fail closed, and replacement cannot inherit trust from a label or similar device name. | Code/security tests do not prove every physical device currently holds the expected key or OS permission state. |
| Remote Assist and Human Takeover | A stale or replayed control session continues after expiry/revocation, a session crosses device identity, capture starts without audit, or automation resumes without an explicit human hand-back. | Owner/session authentication, device/session binding, expiry, capability checks, audit-before-protected-capture, explicit Human Takeover state, and explicit resume semantics are required throughout the session. | CI can verify state-machine and API boundaries; real display/control latency and physical takeover/resume remain separate evidence. |
| Privacy and sensitive displays | Screens, fleet previews, enrollment tokens, or other sensitive panels remain visible when Privacy Blackout is active, or blackout hides a safety warning/Human Takeover state. | Sensitive visual surfaces use the shared blackout boundary; safety warnings and Human Takeover remain visible; blackout never substitutes for authorization, storage protection, or audit. | Blackout protects presentation surfaces only and does not prove runtime memory, OS capture history, external recordings, or third-party telemetry are clean. |
| Restart, reconnect, and replay | Process restart, reconnect, retry, queue recovery, or clock manipulation is used to forget a nonce, revive an expired request, or reset a blocked/approval-required state. | Replay/nonce state is durable for its validity window, clock-skew checks remain fail closed, and blocked/waiting/approval-required states survive recovery. | Simulated restart/reconnect tests remain CODE/UNIT/INTEGRATION evidence; actual host/network/power recovery needs its own evidence class. |
| Protected actions | Prompt injection, model output, pointer/gesture input, a worker result, or a verifier summary attempts to self-authorize billing, credentials, permissions, destructive work, deployment/publication, or security/governance weakening. | Policy and Human Gate authority stay server-side and task-scoped; models/workers/verifiers/pointer/gesture input cannot approve their own protected action. | Correct policy enforcement does not establish that every external system permission is configured safely. |
| Secrets and logs | Credentials, owner/session tokens, private keys, OTPs, or equivalent sensitive values are committed, logged, echoed by scanners, or exposed through diagnostic output. | Repository scanning, logging redaction/no-secret-output contracts, provider-neutral secret handling, and fail-closed unconfigured transports are required. | Static scanning does not prove Git history, process memory, OS logs, third-party observability, or external stores are free of secrets. |
| Provider/cost routing | Failure of a local/free capability silently falls back to a paid or potentially billable provider, or billing uncertainty is treated as free usage. | Paid fallback is prohibited by default; zero-cost status must be verified within existing authorized scope and uncertain billing requires the billing Human Gate. | Provider terms can change, so operational use needs current billing-mode evidence and cannot be inferred from an old test. |

These abuse cases are defense-in-depth constraints. Passing one mitigation never authorizes removing another, and availability recovery never converts a denied security decision into an allowed one.
