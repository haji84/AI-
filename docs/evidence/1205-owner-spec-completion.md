...
The exact candidate has no device credential/protocol/enrollment or permission/network configuration edits. However live GitHub workflow state was read: Broker Refresh357969264 and One-Tap Enrollment E2E356948934 are active and match the changed paths. They update/restart the resident Mac Broker; One-Tap also issues a real fleet registration grant. Vercel Sync356928059 is disabled_manually (do not claim it will trigger). The owner subsequently approved manual-only execution for the two Mac workflows; the exact trigger-only patch is applied and verified in this candidate. This resolves the Mac-hook merge blocker, subject to exact-head CI/review. Manual invocation or Mac setup is not authorized by that approval. Exact state/evidence: [verification](1205/owner-spec-completion-verification.json).

Screenshots use synthetic isolated data: [desktop](1205/owner-spec-desktop.png), [mobile](1205/owner-spec-mobile.png).

## Approved Mac trigger change
Owner: “Macbookは後日追加するから手動実行でOK”. [Scope and exact-delta check](1205/manual-mac-approval.json). The existing Vercel scoped deployment includes credential upserts, which are outside ordinary completion authority; do not set its activation metadata for this approval. The installed Windows release is pinned separately. Main merge/CI alone is not evidence of updating that running installation.
