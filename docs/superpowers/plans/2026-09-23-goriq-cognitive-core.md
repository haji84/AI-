...
### Continuation: explicit pristine Goal DoD refinement (baseline68fd08b)
CI1856 passed the material/history increment. The next audited seam is empty successCriteria from normal GoalController intake. Reuse GoalDraft normalization/readiness and Compass authority; add owner-authenticated bounded explicit criteria adoption only before work exists. Model-proposed semantics are not automatically accepted. Bind currentGoal/full digest, shared execution lease and exact Compass Goal+state CAS; atomically persist an idempotent adopted receipt in existing decisions without schema changes. Preserve title/description/constraints/identity and all history. Reject nonempty criteria, existing cognition/material contracts, pending effects and prior work. Integrate into Tasks UI before material mapping and disable execution while criteria are absent. Tests cover authority/staleness/replay/concurrent state, rollback and actual emptyGoal→criteria→material→verified-download UI. Existing active-work refinement and general semantic inference remain separate gaps.
