# GORIQ portable operation learning — #1216

Baseline d26d67f, CI1858 success. Parent request authorizes bounded software continuation. Production and device state remain untouched.

## Diagnosis and design
The outcome catalog correctly hashes Goal/root/material into exact action IDs. Existing synthesis simultaneously requires the same action ID and two different Goals, so ordinary material intake never produces reusable candidates. Legacy step catalogs can reuse IDs and remain supported.

Reuse existing learning/skill libraries. Add six strict versioned host operation identities (three source inspections, text copy, XLSX creation and DOCX creation). Keep the exact action, Goal, environment, evidence and contract. Operation is selection metadata only; it is not an executable procedure or grant. Models/HTTP cannot assign it. Resolve certified operations only against current eligible low-risk local actions; never copy old paths/input/criteria. Unknown versions fail closed. Failure/correction exclusion remains exact-action scoped.
