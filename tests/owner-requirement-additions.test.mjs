...
test('new adopted capability allocates independent ID and all four artifacts without changing frozen inventory',()=>{
 const db=new CompassStore(':memory:');try{
  ...
  const expectedId=`OWN-${String(b.inventory.allocations.length+1).padStart(3,'0')}`;
  const row=p.bundle.matrix.requirements.at(-1);assert.equal(row.id,expectedId);assert.equal(row.status,'MISSING');assert.equal(row.last_verified_commit,null);
  ...
  assert.match(validateRequirements(missing.matrix,missing.ledger,root,missing.inventory).join(' '),new RegExp(`${expectedId}.*exactly once`));
