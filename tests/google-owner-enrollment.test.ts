...
test("context is 60-second single-use and bound to device key state nonce and PKCE", () => {
  ...
  assert.equal(issued.expiresAt, 1060);
  ...
  const result = store.consume({ ...issued, deviceId: "device_1234567890abcdef", publicKeyThumbprint: publicKeyThumbprint(key), state: "s1", nonce: "n1" }, 1001);
  assert.equal(result.pkceChallenge, "p1");
  assert.throws(() => store.consume({ ...issued, deviceId: "device_1234567890abcdef", publicKeyThumbprint: publicKeyThumbprint(key), state: "s1", nonce: "n1" }, 1002));
});
...
test("expired context fails closed", () => {
  ...
  assert.throws(() => store.consume({ ...issued, deviceId: "device_1234567890abcdef", publicKeyThumbprint: publicKeyThumbprint(key), state: "s", nonce: "n" }, 1061));
});
