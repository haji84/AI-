...
test("trusted PIN client keeps the PIN local and retains passcode fallback", async () => {
  const client = await readFile(new globalThis.URL("../src/app/jarvis/trusted-device-client.ts", import.meta.url), "utf8");
  const login = await readFile(new globalThis.URL("../src/app/jarvis/OwnerLogin.tsx", import.meta.url), "utf8");
...
test("trusted login challenge is HttpOnly and consumed", async () => {
  const challenge = await readFile(new globalThis.URL("../src/app/api/owner-login/trusted/challenge/route.ts", import.meta.url), "utf8");
  const verify = await readFile(new globalThis.URL("../src/app/api/owner-login/trusted/verify/route.ts", import.meta.url), "utf8");
