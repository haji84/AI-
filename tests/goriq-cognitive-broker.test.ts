...
for (const contractMode of ["steps", "outcomes"] as const) test("authenticated Broker executes only configured local work and restores verified cognition after restart: " + contractMode,{timeout:30000},async()=>{
 ...
 const launch=async()=>{child=spawn(process.execPath,["scripts/jarvis-broker.ts"],{windowsHide:true,stdio:"ignore",env:{...process.env,GITHUB_TOKEN:"",GAI_LOCAL_MODEL_NAME:"",JARVIS_BROKER_HOST:"127.0.0.1",JARVIS_BROKER_PORT:String(port),JARVIS_OWNER_TOKEN:token,JARVIS_DB_PATH:join(dir,"broker.sqlite"),JARVIS_COMPASS_DB_PATH:dbPath,GORIQ_LOCAL_WORK_MANIFEST:contractMode === "steps" ? manifestPath : "",GORIQ_LOCAL_OUTCOMES:contractMode === "outcomes" ? manifestPath : "",GORIQ_LOCAL_DATA_ROOT:dataRoot,JARVIS_PUBLIC_BROKER_URL:"",JARVIS_WORKER_INSTALL_URL:"",JARVIS_WORKER_APK_PATH:""}});for(let n=0;n<100;n++){try{if((await fetch(base+"/health",{signal:AbortSignal.timeout(500)})).ok)return;}catch{ /* bounded isolated startup */ }await new Promise(r=>setTimeout(r,50));}throw Error("Isolated Broker startup failed");};
