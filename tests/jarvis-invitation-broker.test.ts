...
test("owner invitation enrolls while window is closed, persists restart and revokes", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    assert.equal((await fetch(base + heartbeatPath, signedRequest("invite-first", heartbeatPath, { status: "ready", capabilities: ["ui-automation", "remote-view", "remote-control", "wake-device"], telemetry: { checkedAt: new Date().toISOString(), accessibilityEnabled: true, remoteProtocol: 1, androidApi: 30, locked: false } }))).status, 200);
    ...
    const wakeResponse = await post(wakePath, { nodeId: "invite-first" }, true);
    assert.equal(wakeResponse.status, 201);
    const wakeTask = (await wakeResponse.json()).task;
    assert.equal(wakeTask.type, "wake-device");
    assert.equal(wakeTask.maxAttempts, 1);
    assert.ok(Date.parse(wakeTask.dispatchBefore) > Date.now());
    assert.ok(Date.parse(wakeTask.dispatchBefore) <= Date.now() + 15_000);
    assert.equal((await (await post(wakePath, { nodeId: "invite-first" }, true)).json()).task.id, wakeTask.id, "concurrent starts share one wake");
    const taskPath = "/api/jarvis/worker/next";
