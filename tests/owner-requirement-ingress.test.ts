...
test("actual authenticated Broker work ingress captures owner requirement and survives restart", {timeout:30000}, async()=>{
 ...
 try{
  ...
  const ideaResponse=await send({text:"たとえば通知機能を追加して",idempotencyKey:"example"});
  const idea=await ideaResponse.json();
  assert.equal(ideaResponse.status,202,`idea ingress rejected: ${JSON.stringify(idea)}`);
  assert.equal(idea.requirement?.state,"IDEA",`idea receipt missing: ${JSON.stringify(idea)}`);
  const chosenResponse=await send({text:"それで進めて",idempotencyKey:"chosen",requirementReferenceId:idea.requirement.id});
  const chosen=await chosenResponse.json();
  assert.equal(chosenResponse.status,202,`explicit reference rejected: ${JSON.stringify(chosen)}`);
  assert.ok(chosen.requirement,`explicit reference receipt missing: ${JSON.stringify(chosen)}`);
  ...
  const nextProposalResponse=await send({text:"文字サイズ機能が欲しい"});
  const nextProposal=await nextProposalResponse.json();
  assert.equal(nextProposalResponse.status,202,`next proposal rejected: ${JSON.stringify(nextProposal)}`);
  const nextChosenResponse=await send({text:"それで進めて"});
  const nextChosen=await nextChosenResponse.json();
  assert.equal(nextChosenResponse.status,202,`next reference rejected: ${JSON.stringify(nextChosen)}`);
  assert.ok(nextChosen.requirement,`next reference receipt missing: ${JSON.stringify(nextChosen)}`);
  assert.match(nextChosen.requirement.statement,/文字/);
