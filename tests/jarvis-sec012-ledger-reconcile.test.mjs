...
const EVIDENCE_IDS = [
  'SEC012-952-CODE',
  'SEC012-952-UNIT',
  'SEC012-952-INTEGRATION',
  'SEC012-952-SECURITY',
];

test('issue #952 reconciliation updates only SEC-012 and stays mirror-validator clean', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'jarvis-952-'));
  try {
    fs.cpSync(ROOT, tmp, {
      recursive: true,
      filter(source) {
    ...
    execFileSync(process.execPath, ['scripts/reconcile-issue-952-sec012.mjs'], { cwd: tmp, stdio: 'pipe' });
    ...
    assert.deepEqual(row.implementation_refs, ['scripts/jarvis-secret-audit.mjs']);
    assert.deepEqual(row.evidence_refs, EVIDENCE_IDS);
    assert.ok(row.test_refs.includes('tests/jarvis-sec012-no-secrets-logs.test.mjs'));
    assert.ok(row.test_refs.includes('tests/jarvis-secret-audit.test.mjs'));
    ...
    const records = after.evidence_records.filter(record => record.id.startsWith('SEC012-952-'));
    ...
    const evidence = fs.readFileSync(path.join(tmp, 'docs/evidence/952-sec012-no-secrets-logs.md'), 'utf8');
    assert.match(evidence, /No device app version or enrollment was changed/);
