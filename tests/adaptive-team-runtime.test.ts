...
const catalog = [{ name: "data.analyze", roles: ["分析"], matchTerms: ["csv", "集計"] }];
...
test("capability outside selected team fails closed and never reaches delegate", async () => {
  ...
  const report = await runAdaptiveTeamGoal({
    ...
    createLoop: createLoopFactory({ ...analyzeAction, capability: "document.write" }),
    createBlueprintId: () => "team-failed-scope",
  });

  assert.equal(counter.calls, 0);
  assert.equal(report.teamOutcome?.ok, false);
  assert.match(report.run?.cycles[0]?.result?.blocker ?? "", /capability_not_in_active_team/);
  assert.equal(memory.get("team-failed-scope")?.failures, 1);
});
...
test("missing required capability blocks before GoalDrivenLoop is created", async () => {
  ...
  const report = await runAdaptiveTeamGoal({
    goal,
    availableCapabilities: catalog,
    requirements: [{ role: "文書", capability: "document.write", reason: "成果物が必要" }],
  ...
  assert.match(report.blockedReason ?? "", /document.write/);
});
