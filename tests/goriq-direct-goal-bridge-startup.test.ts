...
test("Direct Goal Bridge Broker startup diagnostic", { timeout: 15000 }, async () => {
  ...
  const child = spawn(process.execPath, ["scripts/jarvis-broker.ts"], {
    env: { ...process.env, GITHUB_TOKEN: "", JARVIS_BROKER_HOST: "127.0.0.1", JARVIS_BROKER_PORT: String(port), JARVIS_OWNER_TOKEN: randomBytes(32).toString("hex"), JARVIS_DB_PATH: join(dir, "state.sqlite"), JARVIS_COMPASS_DB_PATH: join(dir, "compass.sqlite"), JARVIS_PUBLIC_BROKER_URL: "", JARVIS_WORKER_INSTALL_URL: "", JARVIS_WORKER_APK_PATH: "" },
