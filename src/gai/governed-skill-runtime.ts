...
export interface GovernedSkillEnvironment extends SkillEnvironment {
  platform?: string;
  tools?: string[];
  verifierIds?: string[];
...
export interface SkillExecutionRequirements {
  platforms?: string[];
  tools?: string[];
  verifierIds?: string[];
...
export interface GovernedSkillRecord extends SkillRecord {
  executionRequirements?: SkillExecutionRequirements;
}
...
export function evaluateSkillEligibility(skill: GovernedSkillRecord, env: GovernedSkillEnvironment): SkillEligibility {
  ...
  if (constraints?.connectivity === "online-required" && env.online === false) reasons.push("network:online-required");
  for (const capability of constraints?.capabilities ?? []) if (!env.capabilities?.includes(capability)) reasons.push(`capability:${capability}`);
  for (const resource of constraints?.resources ?? []) if (!env.resources?.includes(resource)) reasons.push(`resource:${resource}`);
  if (constraints?.executionModes?.length && (!env.executionMode || !constraints.executionModes.includes(env.executionMode))) reasons.push(`execution-mode:${env.executionMode ?? "unknown"}`);
  if (constraints?.maxRisk && env.risk && ({ low: 0, medium: 1, high: 2 }[env.risk] > { low: 0, medium: 1, high: 2 }[constraints.maxRisk])) reasons.push(`risk:${env.risk}>${constraints.maxRisk}`);
...
export class GovernedSkillRuntime {
  constructor(private readonly library: PersistentSkillLibrary) {}
  ...
  async select(task: string, env: GovernedSkillEnvironment, limit = 5): Promise<Array<{ skill: GovernedSkillRecord; eligibility: SkillEligibility }>> {
    ...
    return candidates
      .map((skill) => ({ skill: skill as GovernedSkillRecord, eligibility: evaluateSkillEligibility(skill as GovernedSkillRecord, env) }))
      .filter((item) => item.eligibility.eligible)
      .sort((a, b) => b.skill.confidence - a.skill.confidence || a.skill.id.localeCompare(b.skill.id))
      .slice(0, limit);
  }

  async recordRegression(id: string, evidence: string): Promise<SkillRecord | null> {
    if (!evidence.trim()) return null;
    return this.library.quarantine(id, `regression:${evidence.trim()}`);
  }
}
