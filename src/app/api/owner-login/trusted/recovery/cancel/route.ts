...
import { cancelOwnerRecoveryEnrollment } from "../../../../../owner-recovery-service.ts";
...
export async function POST(request: Request) {
  ...
  if (!secret) return NextResponse.json({ message: "復旧コードを取り消せません" }, { status: 503, headers });
  try { await boundedJson(request); }
  catch { return NextResponse.json({ message: "復旧コードを取り消せません" }, { status: 400, headers }); }
  ...
  try {
    return NextResponse.json(await cancelOwnerRecoveryEnrollment(issuerDeviceId, cancelOwnerRecovery), { status: 200, headers });
  } catch {
    return NextResponse.json({ message: "復旧コードを取り消せません" }, { status: 503, headers });
  }
}
