...
export async function POST(request: Request) {
  ...
  const payload = await request.json().catch(() => null) as { text?: unknown; idempotencyKey?: unknown; requirement?: unknown } | null;
  ...
  try {
    const response = await jarvisBrokerFetch("/api/jarvis/admin/work", {
      method: "POST",
      body: JSON.stringify({ text, ...(idempotencyKey ? { idempotencyKey } : {}), ...(payload?.requirement !== undefined ? { requirement: payload.requirement } : {}) }),
    });
