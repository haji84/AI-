...
export default function TeachingControls({serial,sessionId}:{serial:string;sessionId?:string}){
 ...
 async function command(action:string){setBusy(true);try{const r=await fetch('/api/jarvis/remote',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,serial,sessionId,goal:goal.trim()||`手順 ${new Date().toLocaleString('ja-JP')}`,scope,completion:completion.trim()||'停止して保存した時点の画面が表示される',variantId:selected,url})});const b=await r.json();if(!r.ok)throw Error(b.message||b.detail||'教示操作に失敗しました');const run=b.run as TeachingRun|undefined;setMessage(run?`${run.status}：${run.reason??'記録した完了画面まで再現できました'}`:action==='teach-start'?'記録中：GORIQの操作画面から1件分を操作してください。':'保存しました');if(b.variant)setSelected(b.variant.id);await refresh();}catch(e){setMessage(e instanceof Error?e.message:'操作失敗');}finally{setBusy(false);}}
 return <section className="panel jarvis-section jarvis-teaching"><h2>操作を教える</h2><p>全機種共通の手順記憶。ここではGORIQ経由の操作を記録します。端末を直接触った操作や動画だけからの自動学習は未対応です。</p>
 ...
 <div><strong>{recording?'● 手順を記録中':'手順を覚えさせる'}</strong><small>{recording?`${recording.steps.length}操作を保存済み`:'開始 → GORIQで操作 → 停止して保存'}</small></div>
