...
export default function TeachingControls({serial,sessionId}:{serial:string;sessionId?:string}){
 ...
 async function command(action:string){setBusy(true);try{const r=await fetch('/api/jarvis/remote',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,serial,sessionId,goal:goal.trim()||`手順 ${new Date().toLocaleString('ja-JP')}`,scope,completion:completion.trim()||'停止して保存した時点の画面が表示される',variantId:selected,url})});const b=await r.json();if(!r.ok)throw Error(b.message||b.detail||'教示操作に失敗しました');const run=b.run as TeachingRun|undefined;const learning=b.learning;const learningMessage=learning?.status==='error'?learning.message:learning?.status==='candidate'?'Skill候補を保存しました。開始画面に戻し、もう一度再現テストすると認定できます。':learning?.status==='active'?(learning.reused?'認定済みSkillを使いました。':'この端末でSkillを認定しました。'):learning?.status==='quarantined'?'失敗したSkillの再利用を停止しました。':'';setMessage(run?[      `${run.status}：${run.reason??'記録した完了画面まで再現できました'}`,learningMessage].filter(Boolean).join(' '):action==='teach-start'?'記録中：JARVISの操作画面から1件分を操作してください。':action==='teach-correct'?'間違えた操作の前の画面へ戻してください。戻るまで記録を一時停止します。':'保存しました');if(b.variant)setSelected(b.variant.id);await refresh();}catch(e){setMessage(e instanceof Error?e.message:'操作失敗');}finally{setBusy(false);}}
 ...
 <button className="button" disabled={busy||!!recording?.learning?.pendingCorrection} onClick={()=>void command(recording?'teach-finish':'teach-start')}>{busy?'確認中…':recording?'■ 停止して保存':'● 手順の記録開始'}</button>
 {recording&&<button className="button secondary" disabled={busy||!recording.steps.length||!!recording.learning?.pendingCorrection} onClick={()=>void command('teach-correct')}>直前の操作をやり直す</button>}
 {recording?.learning?.pendingCorrection&&<p role="status">訂正待ち：間違えた操作の前の画面へ戻してください。戻るまでの操作は手順へ追加しません。</p>}
