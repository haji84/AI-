...
export default function TeachingLibrary(){
 ...
 async function load(){const r=await fetch('/api/jarvis/teaching',{cache:'no-store'});const b=await r.json();if(!r.ok)throw Error(b.message);setVariants(b.variants);setRuns(b.runs);}
 ...
 <label>操作手順<textarea name="instructions" required maxLength={10000} rows={6}/></label><label>1件の完了条件<input name="completion" required maxLength={500}/></label><button className="button" disabled={busy}>未検証の手順として保存</button><p role="status">{message}</p></form>
