"use client";
import CognitiveGoalCriteria from "./CognitiveGoalCriteria";
...
export default function CognitivePanel() {
  ...
  return <section className="panel jarvis-section" aria-label="GORIQ Cognitive Core">
    ...
    {state && <CognitiveGoalCriteria key={`criteria:${state.goalDigest}`} state={state} disabled={busy || state.busy} onSaved={refresh}/> }
    {state && <CognitiveMaterials key={`materials:${state.goalDigest}`} state={state} disabled={busy} onSaved={refresh}/> }
    <button className="button" disabled={busy || state?.busy || state?.goalComplete || state?.goalRefinementAvailable || !state?.goalId} onClick={() => void continueGoal()}>{state?.goalComplete ? "このGoalは完了しました" : busy ? "処理中…" : "現在のGoalを続ける"}</button>
