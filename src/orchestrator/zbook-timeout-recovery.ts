...
export async function recoverTimedOutGoal(
  ...
  if (run.phase === "QUEUED" && state.status === "IN_PROGRESS" && state.blockers.length === 0) return "ALREADY_QUEUED";
  if (!proof.timeoutConfirmed || run.phase !== "BLOCKED" || state.status !== "BLOCKED"
    || state.blockers.length !== 1 || state.blockers[0] !== "http_code_builder_error"
