...
export function observeAndroidUi(xml:string,profile:DeviceProfile):Observation {
 ...
 for(const n of nodes){const selector=identity(n);const b=n.bounds?.match(/^\[(\d+),(\d+)\]\[(\d+),(\d+)\]$/);if(!b||(!n['resource-id']&&!navigation.test(n.text||n['content-desc']||''))||n.enabled!=="true"||n.clickable!=="true"||nodes.filter(other=>identity(other)===selector).length!==1)continue;const [left,top,right,bottom]=b.slice(1).map(Number);if(right<=left||bottom<=top||right>20000||bottom>20000)continue;targets.push({selector,labelHash:createHash('sha256').update(n.text||n['content-desc']||'').digest('hex'),left,top,right,bottom,x:Math.floor((left+right)/2),y:Math.floor((top+bottom)/2),safeNavigation:!protectedScreen&&navigation.test(n.text||n['content-desc']||'')});}
