...
export const REQUIRED_COUNTS = Object.freeze({ NET: 7, HOST: 8, FLEET: 11, 'DEV-A': 15, 'DEV-I': 8, 'DEV-PC': 7, RA: 22, UI: 38, INT: 17, GEST: 8, AUTO: 31, MEM: 8, OFF: 12, SEC: 19, OPS: 18, ACC: 9, TEACH: 6, MIG: 30 });
...
const physicalRequired = id => /^(NET-00[1-4]|HOST-|DEV-|RA-(?!021)|ACC-|TEACH-|OFF-012|MIG-)/.test(id);
...
export function validateRequirements(matrix, ledger, root) {
  ...
  for (const row of matrix.requirements) {
    ...
    if (/^(SEC-|MIG-)/.test(row.id) && !needs.includes('SECURITY')) fail('SECURITY evidence may not be removed');
