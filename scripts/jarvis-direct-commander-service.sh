...
if command -v lsof >/dev/null 2>&1; then
  for pid in $(lsof -tiTCP:"$JARVIS_COMMANDER_PORT" -sTCP:LISTEN 2>/dev/null || true); do
    ...
    if [[ "$command_line" == *"jarvis-direct-commander"* ]]; then kill "$pid" >/dev/null 2>&1 || true; fi
  done
  for _ in $(seq 1 20); do
    if ! lsof -tiTCP:"$JARVIS_COMMANDER_PORT" -sTCP:LISTEN >/dev/null 2>&1; then break; fi
    sleep 0.2
  done
  if lsof -tiTCP:"$JARVIS_COMMANDER_PORT" -sTCP:LISTEN >/dev/null 2>&1; then echo "Port $JARVIS_COMMANDER_PORT is still occupied by another process" >&2; exit 4; fi
fi
NODE_BIN="$(command -v node || true)"
[[ -x "$NODE_BIN" ]] || { echo "Node.js not found in LaunchAgent PATH" >&2; exit 3; }
cd "$REPO_ROOT"
exec "$NODE_BIN" scripts/jarvis-direct-commander-v5-launcher.mjs
