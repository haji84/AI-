...
interface GapSnapshot { heldoutSuccessRate: number; }
interface CurriculumTask { id: string; sourceCategory: string; difficulty: number; prompt: string; answer: string; promptSha256: string; }

const outDir = path.resolve(".gai-results");
const refDir = path.resolve(".gai-reference/r1");
await mkdir(outDir, { recursive: true });
const taxonomy = JSON.parse(await readFile(path.join(refDir, "r1-failure-taxonomy.json"), "utf8")) as Taxonomy;
const gap = JSON.parse(await readFile(path.join(refDir, "r1-agi-gap-snapshot.json"), "utf8")) as GapSnapshot;
if (!taxonomy.failures) throw new Error("R15 requires verified R1 failures; taxonomy contains none");

const normalize = (text: string) => text.trim().toLowerCase().replace(/\s+/g, " ");
const hash = (text: string) => crypto.createHash("sha256").update(normalize(text)).digest("hex");
const heldoutHashes = new Set<string>();
...
function makeTask(category: string, index: number, difficulty: number): Omit<CurriculumTask, "promptSha256"> {
...
const generated: CurriculumTask[] = [];
