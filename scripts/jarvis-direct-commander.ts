...
function page(key: string) {
  ...
  <section class="panel"><strong>クイック操作</strong><div class="grid" style="margin-top:12px"><button class="btn" onclick="send('wake-device',{})">画面を起こす</button><button class="btn" onclick="send('device-status',{})">端末状態</button><button class="btn primary" onclick="spreadsheetTask()">スプレッドシート作業</button><button class="btn" onclick="send('open-app',{packageName:'com.android.chrome'})">Chrome</button><button class="btn" onclick="send('open-app',{packageName:'com.google.android.apps.maps'})">Google Maps</button><button class="btn" onclick="send('launch-settings',{screen:'wifi'})">Wi-Fi設定</button><button class="btn" onclick="send('ui-sequence',{steps:[{action:'home'}]})">ホーム</button><button class="btn" onclick="send('ui-sequence',{steps:[{action:'back'}]})">戻る</button></div></section>
  <section id="sheetTask" class="panel"><strong>TikTok Lite スプレッドシート作業</strong><p class="sub">Sheetsを開き、「TikTok Lite」ファイルへ移動して、指定した項目を上から順番に押します。</p><label>ファイル名</label><input id="sheetFile" class="field" value="TikTok Lite"><label>押す項目（1行に1つ、上から順番）</label><textarea id="sheetTargets" rows="6" placeholder="例：\n春巻QR\n5~56サッカー\nなおきQR"></textarea><label class="check"><input id="sheetBack" type="checkbox">各項目を押したあと「戻る」を1回実行</label><p class="hint">リンクを開いたあと毎回シートへ戻したい場合だけオン。</p><button class="btn primary" style="margin-top:12px;width:100%" onclick="spreadsheetTask()">この順番でタスク実行</button></section>
