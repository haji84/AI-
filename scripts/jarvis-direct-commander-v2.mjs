...
function task(type, payload, node, suffix) {
  return { type, payload, targetNodeId: node, idempotencyKey: `${type}:${Date.now()}:${suffix}` };
}
async function choiceFlow(input) {
  ...
  const openTask = await broker('/api/jarvis/admin/tasks', { method: 'POST', body: JSON.stringify(task('open-app', { packageName: 'com.google.android.apps.docs.editors.sheets' }, node, 'sheet-open')) });
  const steps = [
    { action: 'wait', ms: 1200 },
  ...
  const sequenceTask = await broker('/api/jarvis/admin/tasks', { method: 'POST', body: JSON.stringify(task('ui-sequence', { steps }, node, 'sheet-flow')) });
...
async function api(path,opt={}){const r=await fetch(path,{...opt,headers:{'X-JARVIS-Commander-Key':key,...(opt.headers||{})}});const raw=await r.text();let b;try{b=raw?JSON.parse(raw):{};}catch{b={message:raw};}if(!r.ok)throw new Error(b.message||('HTTP '+r.status));return b;}
function save(){try{localStorage.setItem('jarvisAssignmentsV2',JSON.stringify(assignments));}catch{}}
function load(){try{assignments=JSON.parse(localStorage.getItem('jarvisAssignmentsV2')||'{}')||{};}catch{assignments={};}}
function target(){return $('device').value||'';}
async function refresh(){try{const s=await api('/api/state');fleet=Array.isArray(s.fleet)?s.fleet:[];$('broker').textContent='Broker 接続中';$('broker').className='pill ok';$('count').textContent='端末 '+fleet.length;const keep=$('device').value;$('device').innerHTML=fleet.map(n=>'<option value="'+esc(n.id)+'">'+esc(n.label||n.id)+' · '+esc(n.status||'unknown')+'</option>').join('');if(keep&&fleet.some(n=>n.id===keep))$('device').value=keep;render();if($('status').textContent==='起動中…')show('操作可能 ✅');}catch(e){$('broker').textContent='Broker エラー';$('broker').className='pill bad';show('接続エラー: '+e.message);}}
async function accessInfo(){try{const x=await api('/api/access-info');$('lanUrl').textContent='iPhone / ZBook 用: '+x.lanUrl;}catch(e){$('lanUrl').textContent='URL取得失敗: '+e.message;}}
async function send(type,payload){const node=target();if(!node)return show('操作する端末を選んでください');try{const b=await api('/api/task',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type,payload,targetNodeId:node,idempotencyKey:type+':'+Date.now()})});show('受付完了 ✅\n'+type+'\n状態: '+((b.task||b).status||'queued'));}catch(e){show('操作エラー: '+e.message);}}
...
async function run(){const jobs=fleet.filter(n=>assignments[n.id]?.first&&assignments[n.id]?.second);if(!jobs.length)return show('実行設定された端末がありません');show(jobs.length+'台へタスク送信中…');const out=[];for(const n of jobs){const a=assignments[n.id];try{const b=await api('/api/spreadsheet-choice-flow',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({targetNodeId:n.id,firstChoice:a.first,secondChoice:a.second})});out.push('✅ '+(n.label||n.id)+': '+b.first.label+' → '+b.second.label);}catch(e){out.push('❌ '+(n.label||n.id)+': '+e.message);}}show(out.join('\n'));}
function boot(){load();$('wake').addEventListener('click',()=>send('wake-device',{}));$('home').addEventListener('click',()=>send('ui-sequence',{steps:[{action:'home'}]}));$('wifi').addEventListener('click',()=>send('launch-settings',{screen:'wifi'}));$('apply').addEventListener('click',apply);$('run').addEventListener('click',run);refresh();accessInfo();setInterval(refresh,3000);}
...
createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/health') return json(res, 200, { ok: true, service: 'jarvis-direct-commander-v2', host: HOST, port: PORT });
    if (req.method === 'GET' && url.pathname === '/' && privateHost(req.headers.host)) {
      res.writeHead(302, { Location: `/c/${encodeURIComponent(KEY)}`, 'Cache-Control': 'no-store' }); return res.end();
    }
    if (req.method === 'GET' && url.pathname === `/c/${encodeURIComponent(KEY)}`) return text(res, 200, 'text/html', PAGE);
    if (req.method === 'GET' && url.pathname === '/commander-v2.js') return text(res, 200, 'text/javascript', CLIENT);
    if (!authorized(req, url)) return json(res, 401, { message: 'commander access denied' });
    if (req.method === 'GET' && url.pathname === '/api/access-info') return json(res, 200, { lanIp: lanIp(), lanUrl: `http://${lanIp()}:${PORT}/` });
...
}).listen(PORT, HOST, () => console.log(`[jarvis-direct-commander-v2] listening on http://${HOST}:${PORT}`));
