...
if (!decision.accepted || decision.evidence.length === 0) throw new Error(`external evidence rejected: ${decision.reasons.join("; ")}`);
...
await writeFile(output, JSON.stringify(decision.evidence, null, 2), "utf8");
console.log(JSON.stringify({ accepted: true, output, evidenceIds: decision.evidence.map((item) => item.id) }, null, 2));
