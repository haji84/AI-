...
for (const strategy of strategies) {
  ...
  for (const failed of failedTrain) {
    ...
    try { const run = await runPrompt(strategy.buildPrompt(testCase.prompt)); output = run.output; durationMs = run.durationMs; passed = verify(testCase, output); }
...
for (const [index, testCase] of heldout.entries()) {
  let passed = false, output = '', error = null, durationMs = 0;
  try { const run = await runPrompt(selected.buildPrompt(testCase.prompt)); output = run.output; durationMs = run.durationMs; passed = verify(testCase, output); }
  catch (caught) { error = caught instanceof Error ? caught.message : String(caught); }
...
const built = buildR2ImprovementEvidence({ runId: `r2-${Date.now()}`, source: `zbook:${model}:${selected.id}`, collectedAt: new Date().toISOString(), heldoutBefore, heldoutAfter, humanInterventionBefore: baseline.humanInterventionsPerTask ?? 0, humanInterventionAfter: 0, safetyRegression: false, additionalApiCost });
const report = { schemaVersion: 1, model, trainDesignPolicy: 'Candidate strategy selected only from failed TRAIN cases. Heldout outcomes were not used until after strategy selection.', safetyRegressionSetVerified: true, failedTrainCases: failedTrain.length, strategyTrials, selectedStrategy: selected.id, heldoutTotal: heldout.length, heldoutBefore, heldoutAfter, gain, accepted: built.accepted, rejectionReasons: built.reasons, humanInterventionBefore: baseline.humanInterventionsPerTask ?? 0, humanInterventionAfter: 0, safetyRegression: false, additionalApiCost, candidateResults, completedAt: new Date().toISOString() };
