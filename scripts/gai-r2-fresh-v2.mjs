...
async function runPrompt(prompt){
  ...
  const response=await fetch(`${endpoint}/api/generate`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model,prompt,stream:false,keep_alive:'30m',options:{temperature:0,num_predict:256}}),signal:AbortSignal.timeout(120000)});
...
async function evaluate(cases,strategy,label){
  ...
  for(const [index,testCase] of cases.entries()){
    ...
    if(!row){
      ...
      row={key,id:testCase.id,category:testCase.category,split:testCase.split,passed,output,error,durationMs,humanInterventions:0,additionalApiCost:0}; cache.set(key,row); persist();
    }
    results.push(row);
    const elapsed=Date.now()-started, done=index+1, etaMs=done?Math.round(elapsed/done*(cases.length-done)):null;
    console.log(`[R2v2 ${label} ${done}/${cases.length}] ${testCase.id} ${row.passed?'PASS':'FAIL'} ${strategy.id} cached=${cache.has(key)} etaSec=${etaMs===null?'?':Math.ceil(etaMs/1000)}`);
  }
  return results;
}
...
// Candidate is frozen here. HELDOUT is touched only after this point. Cache keys include
// strategy and case identity, so resume never substitutes another candidate's answer.
