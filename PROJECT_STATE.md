...
STATUS: DYNAMIC_CAPABILITY_REPLANNING_IMPLEMENTATION
LAST_UPDATED: 2026-09-13
CURRENT_EPIC: One-front-door general autonomous AI with dynamic reusable and runtime-expandable capability orchestration
ACTIVE_ISSUES: #477, #320, #315, #316, #317, #318, #319, #321
OPEN_PRS: dynamic capability replanning + empirical research/device-control work may proceed independently when scopes do not conflict
...
ARCHITECTURE_REFERENCE: docs/architecture/general-autonomous-ai-dynamic-orchestration.md + docs/architecture/team-organizational-memory.md + docs/architecture/adaptive-team-runtime.md + docs/architecture/dynamic-capability-replanning.md
LAST_SUCCESSFUL_CI: adaptive team runtime PR #474 passed lint/test/build/health plus repository-guard and merged on 2026-09-13; Issue #477 requires fresh CI before merge
...
GAI_MISSION: develop and research a measurable general autonomous intelligence architecture that can plan, act, remember, learn, verify, transfer experience, dynamically compose and expand capabilities, retain successful organization patterns, and improve itself without claiming AGI prematurely
GAI_CORE_LOOP: Goal -> DoD -> Current State -> Context partition -> Plan -> Recall/assemble team -> Predict -> Act -> detect capability gap -> necessity/risk/gate check -> bounded replan/recruit -> Observe -> Verify -> Repair -> Deliver -> Write-back -> Record team outcome -> Persist organization memory -> Benchmark -> Learn -> Abstract -> Transfer -> Diagnose -> Hypothesize -> Experiment -> Held-out evaluate -> Accept/Reject -> Re-evaluate
...
GAI_NEXT_PHASE: complete #477 bounded dynamic capability replanning, then execute real Benchmark Suite/Baseline runs on configured workers and measure transfer/intervention rates
...
NEXT_PRIORITY: complete #477 and merge after fresh CI; Human Gate categories remain non-negotiable and must not be auto-executed
HUMAN_APPROVAL_PENDING: none for ordinary LOW/MEDIUM implementation; production, secrets, permissions, billing, destructive, governance/security weakening, and external-publication gates remain in force
