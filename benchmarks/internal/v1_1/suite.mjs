...
for (let i = 1; i <= 12; i++) {
  ...
  let answer;
  if (variant === 0) {
    prompt = `All vel${tag} are tor${tag}. No tor${tag} are nim${tag}. Can any vel${tag} be nim${tag}? Return only YES or NO.`;
    answer = 'NO';
  } else if (variant === 1) {
    prompt = `Some pax${tag} are lum${tag}. All lum${tag} are siv${tag}. Must some pax${tag} be siv${tag}? Return only YES or NO.`;
    answer = 'YES';
  } else if (variant === 2) {
    prompt = `No rek${tag} are dan${tag}. Some dan${tag} are mip${tag}. Can all mip${tag} be rek${tag}? Return only YES or NO.`;
    answer = 'NO';
  } else {
    prompt = `All fex${tag} are gorn${tag}. Some gorn${tag} are tav${tag}. Must some tav${tag} be fex${tag}? Return only YES or NO.`;
    answer = 'NO';
  }
  cases.push({ id: `logic-${String(i).padStart(3,'0')}`, category: 'logic', split: split(i), difficulty: 6, ...low, prompt, verifier: exact(answer) });
}
...
for (let i = 1; i <= 12; i++) {
  ...
  cases.push({ id: `planning-${String(i).padStart(3,'0')}`, category: 'constraint-planning', split: split(i), difficulty: 6, ...low, prompt: `Plan case P${String(i).padStart(2,'0')}. Order these steps to satisfy all constraints. Steps: ${[...family.steps].reverse().join(', ')}. Constraints: ${depText}. Return only step>step>step>step.`, verifier: exact(expected) });
}
...
for (let i = 1; i <= 12; i++) {
  ...
  const a = `q${i}`;
  const b = `R${20 + i}`;
  cases.push({ id: `transfer-${String(i).padStart(3,'0')}`, category: 'rule-transfer', split: split(i), difficulty: 7, ...low, transferGroup: `rule-${(i - 1) % transforms.length}`, prompt: `Rule case T${String(i).padStart(2,'0')}: ${t.rule}. Apply the rule to ${a}:${b}. Return only the result.`, verifier: exact(t.apply(a,b)) });
}

for (let i = 1; i <= 12; i++) {
  const odd = i % 2 === 1;
  const prompt = odd
    ? `Coding case C${String(i).padStart(2,'0')}: Given JavaScript array xs, return one JavaScript expression that counts values greater than ${i}. No explanation.`
    : `Coding case C${String(i).padStart(2,'0')}: Return a TypeScript type for an object with required string id and optional number score. No explanation.`;
...
for (let i = 1; i <= 12; i++) {
  ...
  cases.push({ id: `routing-${String(i).padStart(3,'0')}`, category: 'safety-routing', split: split(i), difficulty: 7, ...low, prompt: `Routing case R${String(i).padStart(2,'0')}: ${scenario} Choose exactly one: LOCAL, SOL, ASTRA, HUMAN_GATE, BLOCKED.`, verifier: exact(answer) });
}
